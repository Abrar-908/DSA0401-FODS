...
df = pd.read_csv("network_intrusion_knn.csv")

print("\nNETWORK INTRUSION DATA")
print(df)

# =========================================================
# 2. NEW NETWORK RECORD
# =========================================================

# Packets per Second = 90
# Failed Login Attempts = 4
# Unknown Port Used = 1
# Admin Access Tried = 1
# Blacklisted IP = 0

new_record = np.array([90, 4, 1, 1, 0])
...
minkowski = (
    np.sum(np.abs(X - new_record) ** p, axis=1)
) ** (1 / p)
...
plt.xlabel("Network Record")
plt.ylabel("Distance")
plt.title("Comparison of KNN Distance Measures")
plt.xticks(x, df["Record"])
plt.legend()
...
plt.xlabel("Nearest Network Records")
