...
df = pd.read_csv("loan_approval_knn.csv")
...
new_applicant = np.array([55000, 700, 1, 0, 0])
...
euclidean = np.sqrt(
    np.sum((X - new_applicant) ** 2, axis=1)
)
...
minkowski = (
    np.sum(np.abs(X - new_applicant) ** p, axis=1)
) ** (1 / p)
